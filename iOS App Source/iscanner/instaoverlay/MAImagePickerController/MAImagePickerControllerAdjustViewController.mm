...
{
    NSLog(@"View Bounds : %@",NSStringFromCGRect(_imageAdjustView.bounds));
    NSLog(@"View frame : %@",NSStringFromCGRect(_imageAdjustView.frame));
    UIView *backgroundView = [[UIView alloc] initWithFrame:_imageAdjustView.bounds];
    [backgroundView setBackgroundColor:[UIColor blackColor]];
    
    if(SYSTEM_VERSION_LESS_THAN(@"8.0")){
       _imageAdjustView.transform = CGAffineTransformMakeRotation(M_PI / 2);
    }
    
    [_imageAdjustView addSubview:backgroundView];
    NSLog(@"View after clicking photo = %@",NSStringFromCGRect(_imageAdjustView.frame));
    NSLog(@"_sourceImageView :  width = %f height = %f",_imageAdjustView.bounds.size.width,_imageAdjustView.bounds.size.height - kCameraToolBarHeight);
    if(!_sourceImageView)
    _sourceImageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, _imageAdjustView.bounds.size.width, _imageAdjustView.bounds.size.height - kCameraToolBarHeight)];
    
    if(SYSTEM_VERSION_LESS_THAN(@"8.0")){
       _sourceImageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0,_imageAdjustView.bounds.size.height,_imageAdjustView.bounds.size.width - kCameraToolBarHeight)];
    }
    ...
    [_imageAdjustView addSubview:_sourceImageView];
    /*Suryakant: transform ImageView to make it work for landscape */
     _sourceImageView.transform = CGAffineTransformMakeRotation(-M_PI/2);
    //saravanan
    [self fitImageView:_adjustedImage CONTAINER:_sourceImageView];
    
    
    NSLog(@"_sourceImageView : contentFrame = %@",NSStringFromCGSize(_adjustedImage.size));
    
    NSLog(@"contentFrame = %@",NSStringFromCGRect(_sourceImageView.contentFrame));
    _adjustRect= [[MADrawRect alloc] initWithFrame:_sourceImageView.contentFrame];
    [_adjustRect setBackgroundColor:[UIColor clearColor]];
    
    [[_imageAdjustView superview] addSubview:_adjustRect];
    //[_imageAdjustView bringSubviewToFront:_adjustRect];
    /*Suryakant: transform ImageView to make it work for landscape*/
    _adjustRect.transform = CGAffineTransformMakeRotation(-M_PI/2);

    
    /*_adjustToolBar = [[UIToolbar alloc] initWithFrame:CGRectMake(0, _imageAdjustView.bounds.size.height - kCameraToolBarHeight, _imageAdjustView.bounds.size.width, kCameraToolBarHeight)];
    ...
    [_imageAdjustView addSubview:_adjustToolBar];*/
    ...
    [self AdjustRect];
# warning dsfgdsfdsfgdfsfdgd
    [NSTimer scheduledTimerWithTimeInterval:2.0 target:self selector:@selector(removeAdjustRactView) userInfo:nil repeats:NO];
    
    [self confirmedImage];
}
- (void)removeAdjustRactView{
    [_adjustRect removeFromSuperview];
...
{
    NSLog(@"MAImagePicketControllerAdjustViewContrller : popCurrentViewController");
    [[NSNotificationCenter defaultCenter] postNotificationName:kResetFrameCapture object:nil userInfo:nil];
...
- (void)detectEdges
{
   ...
   // NSLog(@"Largest square :  wid = %@",largest_square);
...
//        NSLog(@"Points : min = %d max = %d",min,max);
...
//        NSLog(@"Points : minIndex = %d maxIndex = %d",minIndex,maxIndex);
//        NSLog(@"missingIndexOne = %d",missingIndexOne);
        ...
        NSLog(@"missingIndexTwo = %d",missingIndexTwo);
        ...
        {
            NSLog(@"4th Point Found");
            //4rd Point Found
            [sortedPoints setObject:[[points objectAtIndex:missingIndexOne] objectForKey:@"point"] forKey:@"1"];
            [sortedPoints setObject:[[points objectAtIndex:missingIndexTwo] objectForKey:@"point"] forKey:@"3"];
        }
        
//        NSLog(@"sortedPoints = %@",sortedPoints);
...
- (void)confirmedImage
{
    ...
    if(!_finalView)
    _finalView = [[MAImagePickerFinalViewController alloc] init];
    _finalView.sourceImage = _adjustedImage;
    _finalView.imageFrameEdited = edited;
    _finalView.finalImagePickerView = _imageAdjustView;
    [_finalView setupFinalImagePickerview];
...
{
    NSLog(@"find_largest_square : size = %lu",squares.size());
    if (!squares.size())
    {
        // no squares detected
        return;
...
//    NSLog(@"find_largest_square : squares = %lu",squares.size());
