...
- (void)setupFinalImagePickerview
{  
    [self setupToolbar];
    [self setupEditor];
    
    [_finalImagePickerView setBackgroundColor:[UIColor blackColor]];
    
    _adjustedImage = _sourceImage;
    
    _finalImageView = [[UIImageView alloc] init];
    [_finalImageView setFrame:CGRectMake(0, 0, _finalImagePickerView.bounds.size.width+50, 30+_finalImagePickerView.bounds.size.height - (kCameraToolBarHeight + 70))];
    ...
    // Suryakant: transform ImageView to make it work for landscape
    _finalImageView.transform = CGAffineTransformMakeRotation(-M_PI/2);
    [_finalImagePickerView addSubview:imgScrollView];
    ...
    [_finalImagePickerView addSubview:_progressIndicator];
    
    
    [self setupSettingIcons];
    
    [self comfirmFinishedImage];
}
...
{
    NSLog(@"comfirmFinishedImage : %@",NSStringFromCGSize(_finalImageView.image.size));
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        UIImageWriteToSavedPhotosAlbum(_finalImageView.image, nil, nil, nil);
    });
...
//    if (![[NSUserDefaults standardUserDefaults]stringForKey:@"serverUrl"].length) {
//        NSLog(@"serverUrl is empty");
//        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Error"
//                                                        message:@"Please Update the Url to upload"
//                                                       delegate:nil
...
//    }else {
        ...
        NSData *imageData = UIImageJPEGRepresentation(_finalImageView.image, 1.0);
        
        AFHTTPRequestOperationManager *manager = [AFHTTPRequestOperationManager manager];
        manager.responseSerializer = [AFHTTPResponseSerializer serializer];
        manager.requestSerializer = [AFHTTPRequestSerializer serializer];
        [[NSNotificationCenter defaultCenter]postNotificationName:@"uploadingStart" object:nil userInfo:nil];

        [manager POST:[[NSUserDefaults standardUserDefaults]stringForKey:@"serverUrl"]  parameters:NULL constructingBodyWithBlock:^(id<AFMultipartFormData> formData) {
            [formData appendPartWithFileData:imageData
                                        name:@"file"
                                    fileName:@"lic_img.jpg"
                                    mimeType:@"image/jpeg"];
            // etc.
            ...
            int confidanceValue = [[[json objectForKey:@"meanConfidence"] objectAtIndex:0]  intValue];
            if(confidanceValue >= 65){
                NSLog(@"String");
                
                [[NSNotificationCenter defaultCenter]postNotificationName:@"MAImageAccepted" object:nil userInfo:json];
            }
            else{
                [[NSNotificationCenter defaultCenter]postNotificationName:@"stop_spinner" object:nil userInfo:json];
                [[NSNotificationCenter defaultCenter]postNotificationName:@"MAImageNotHaveRequiredConfidanceLevel" object:nil userInfo:json];
            }
            
        } failure:^(AFHTTPRequestOperation *operation, NSError *error) {
            NSDictionary *dic=[NSDictionary dictionaryWithObjectsAndKeys:
                               
                               [NSString stringWithFormat:@"Status Code : %ld",(long)[operation.response statusCode]],@"statusCode",
                               
                               [[error userInfo] descriptionWithLocale:@"NSLocalizedDescription"], @"responseString",
                               
                               nil];
            
            appDelegate.isUploading=NO;
            [[NSNotificationCenter defaultCenter]postNotificationName:@"stop_spinner" object:nil userInfo:dic];
            [[NSNotificationCenter defaultCenter]postNotificationName:@"MAImageNotHaveRequiredConfidanceLevel" object:nil userInfo:dic];
...
- (void) updateImageView
{
    ...
    [_progressIndicator stopAnimating];
    [_finalImagePickerView setUserInteractionEnabled:YES];
}
...
- (IBAction) filterChanged:(id) sender withEvent:(UIEvent *) event
{
    ...
    {
        [_finalImagePickerView setUserInteractionEnabled:NO];
...
- (void)setupEditor
{
    UIView *editorView = [[UIView alloc] initWithFrame:CGRectMake(0, _finalImagePickerView.bounds.size.height - (kCameraToolBarHeight + 60), _finalImagePickerView.bounds.size.width, 60)];
    
    UIImageView *editorViewBackground = [[UIImageView alloc] initWithFrame:CGRectMake(0, 31, _finalImagePickerView.bounds.size.width, 29)];
    ...
    [_finalImagePickerView addSubview:editorView];
}

- (void)setupToolbar
{
    UIToolbar *finishToolBar = [[UIToolbar alloc] initWithFrame:CGRectMake(0, _finalImagePickerView.bounds.size.height - kCameraToolBarHeight, _finalImagePickerView.bounds.size.width, kCameraToolBarHeight)];
    ...
    [_finalImagePickerView addSubview:finishToolBar];
}
...
- (void)rotateImage
{
    ...
    UIImageWriteToSavedPhotosAlbum(_adjustedImage, nil, nil, nil);
