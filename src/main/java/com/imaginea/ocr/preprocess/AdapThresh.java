...
public class AdapThresh extends Thread {
	 ...
	 *
	 * @param src
	 *            pixel array representing image to be thresholded
	 * @param width
	 *            width of the image in pixels
	 * @param height
	 *            height of the image in pixels
	 * @param size
	 *            the size of the neigbourhood used in finding the threshold
	 * @param con
	 ...
	 *            the size of the neigbourhood used in finding the threshold
	 * @param con
	 *            the constant value subtracted from the median
	 * @return a thresholded pixel array of the input image array
	 */
		...
		for (int j = 0; j < i_h; j++) {
			for (int i = 0; i < i_w; i++) {
				...
				for (int k = 0; k < size; k++) {
					for (int l = 0; l < size; l++) {
						try {
							values[count] = tmp_2d[(i - ((int) (size / 2)) + k)][(j
									- ((int) (size / 2)) + l)];
							count++;
						}
						// If out of bounds then ignore pixel
